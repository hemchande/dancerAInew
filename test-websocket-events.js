...
console.log('🧪 Testing WebSocket events on port 8001...');
...
socket.onAny((eventName, ...args) => {
  ...
  testResults.eventsReceived.push({ eventName, data: args });
});
...
socket.on('connect', () => {
  ...
  socket.emit('register', { user_id: 'test-user-events' });
});
...
function printTestSummary() {
  ...
  if (testResults.eventsReceived.length > 0) {
    ...
    testResults.eventsReceived.forEach((event, index) => {
      console.log(`   ${index + 1}. "${event.eventName}" - ${JSON.stringify(event.data)}`);
    });
  }
...
// Print summary and exit after 8 seconds
setTimeout(() => {
  printTestSummary();
  socket.disconnect();
  process.exit(0);
